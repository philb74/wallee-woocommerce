...
/**
 *
 * WC_Wallee_Service_Line_Item Class
 *
 * Wallee
...
class WC_Wallee_Service_Line_Item extends WC_Wallee_Service_Abstract {
	...
	public function get_items_from_session() {
		...
		return WC_Wallee_Helper::instance()->cleanup_line_items( $combined, $cart->total, $currency );
	}
	...
	protected function create_product_line_items_from_session( WC_Cart $cart, $currency ) {
		...
		foreach ( $cart->get_cart() as $values ) {
			...
			$items[] = apply_filters( 'wc_wallee_modify_line_item_product_session', $this->clean_line_item( $line_item ), $values );
		}
		return $items;
	}
	...
	protected function create_fee_lines_items_from_session( WC_Cart $cart, $currency ) {
		...
		foreach ( $cart->get_fees() as $fee ) {
			...
			$fees[] = apply_filters( 'wc_wallee_modify_line_item_fee_session', $this->clean_line_item( $line_item ), $fee );
		}
		return $fees;
	}
	...
	protected function create_shipping_line_items_from_session( $packages, $chosen_methods, $currency ) {
		...
		foreach ( $packages as $package_key => $package ) {
			if ( isset( $chosen_methods[ $package_key ], $package['rates'][ $chosen_methods[ $package_key ] ] ) ) {
				...
				$shippings[] = apply_filters( 'wc_wallee_modify_line_item_shipping_session', $this->clean_line_item( $line_item ), $shipping_rate );
			}
		}
		return $shippings;
	}
	...
	private function create_coupon_line_item( $coupon, float $amount = 0 ) {
		...
		$line_item->setName( sprintf( '%s: %s', WC_Wallee_Packages_Coupon_Discount::COUPON, $coupon->get_code() ) );
		$line_item->setQuantity( 1 );
		$line_item->setShippingRequired( false );
		$line_item->setSku( $sku, 200 );
		$line_item->setType( \Wallee\Sdk\Model\LineItemType::DISCOUNT );
	...
	public function get_items_from_order( WC_Order $order ) {
		...
		$total = apply_filters( 'wc_wallee_modify_total_to_check_order', $order->get_total(), $order );
		return WC_Wallee_Helper::instance()->cleanup_line_items( $items, $total, $order->get_currency() );
	}
	...
	protected function create_product_line_items_from_order( WC_Order $order ) {
		...
		foreach ( $order_items as $item ) {
			...
			$coupon_discount = $item->get_meta( '_wallee_coupon_discount_line_item_discounts' );
			if ( $coupon_discount > 0 ) {
				//there is some coupon discount applied
				$amount = $amount_including_tax;
			...
			$items[] = apply_filters( 'wc_wallee_modify_line_item_product_order', $this->clean_line_item( $line_item ), $item );
		}
		return $items;
	}
	...
	protected function create_fee_lines_items_from_order( WC_Order $order ) {
		...
		foreach ( $order->get_fees() as $fee ) {
			...
			$fees[] = apply_filters( 'wc_wallee_modify_line_item_fee_order', $this->clean_line_item( $line_item ), $fee );
		}
		return $fees;
	}
	...
	protected function create_shipping_line_items_from_order( WC_Order $order ) {
		...
		foreach ( $order->get_shipping_methods() as $shipping ) {
			...
			$shippings[] = apply_filters( 'wc_wallee_modify_line_item_shipping_order', $this->clean_line_item( $line_item ), $shipping );
		}
		return $shippings;
	}
	...
	public function get_items_from_backend( array $backend_items, $amount, WC_Order $order ) {
		...
		return WC_Wallee_Helper::instance()->cleanup_line_items( $combined, $amount, $order->get_currency() );
	}
	...
	protected function create_product_line_items_from_backend( array $backend_items, WC_Order $order ) {
		...
		foreach ( $order->get_items() as $item_id => $item ) {
			...
			$items[] = apply_filters( 'wc_wallee_modify_line_item_product_backend', $this->clean_line_item( $line_item ), $item );
		}
		return $items;
	}
	...
	protected function create_fee_lines_items_from_backend( array $backend_items, WC_Order $order ) {
		...
		foreach ( $order->get_fees() as $fee_id => $fee ) {
			...
			$fees[] = apply_filters( 'wc_wallee_modify_line_item_fee_backend', $this->clean_line_item( $line_item ), $fee );
		}
		return $fees;
	}
	...
	protected function create_shipping_line_items_from_backend( array $backend_items, WC_Order $order ) {
		...
		foreach ( $order->get_shipping_methods() as $shipping_id => $shipping ) {
			...
			$shippings[] = apply_filters( 'wc_wallee_modify_line_item_shipping_backend', $this->clean_line_item( $line_item ), $shipping );
		}
		return $shippings;
	}
	 ...
	 *
	 * @param \Wallee\Sdk\Model\LineItemCreate $line_item line item.
	 * @return \Wallee\Sdk\Model\LineItemCreate
	 */
	...
	protected function get_taxes( array $rates_or_ids ) {
		...
		foreach ( array_keys( $rates_or_ids ) as $rate_id ) {
			$tax_rates[] = new \Wallee\Sdk\Model\TaxCreate(
				array(
	...
	private function get_base_attributes( $product_id ) {
		...
		foreach ( $products_to_check as $id ) {
			...
			if ( $wpm_gtin_code ) {
				$wpm_gtin_attribute = new \Wallee\Sdk\Model\LineItemAttributeCreate();
